...
- **remote** (default) — you drive rounds. Omit `moderator_model`.
- **autonomous** — set `moderator_model` and `rounds` (1–14). The AI moderator runs the full arc. Poll `get_session` until `status: "ready"`.
- **single_shot** — just call `create_deliberation` with `rounds: 1` and don't follow up.
...
When selecting participant models for a deliberation yourself, default to a **frontier-tier panel** (Claude Opus, GPT-5.4 or equivalent, Gemini 3.1 Pro) for contested architecture, plan-review, or strategy work. Use nano/fast variants (GPT-5.4-nano, Gemini Flash, Haiku) only when the user explicitly asks for a cheap gut-check. The cost delta is usually <$0.30 and the signal gap is large.
